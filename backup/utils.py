...
class TextDataset(object):
    def clean_text(self, num='substitute'):
        ...
        for i, doc in enumerate(self.documents):
            ...
            doc = re.sub('[^a-z]', ' ', doc)
            doc = ' '.join(
                doc.split())  # same as doc = re.sub('\s{2,}', ' ', doc)
...
class TextRCV1(TextDataset):
    def __init__(self, **params):
        ...
        self.class_names = dataset.target_names
        assert len(
            self.class_names) == 103  # 103 categories according to LYRL2004
    ...
    def show_doc_per_class(self, print_=False):
        ...
        docs_per_class = np.array(self.target.astype(
            np.uint64).sum(axis=0)).squeeze()
        print(('categories ({} assignments in total)'.format(
            docs_per_class.sum())))
        if print_:
    ...
    for i, c in enumerate(clf):
        if i not in omit:
            ...
            train_accuracy.append(
                '{:5.2f}'.format(100 * sklearn.metrics.accuracy_score(
...
def grid_search(params, grid_params, train_data, train_labels, val_data,
    ...
    for i, grid_params in enumerate(grid):
        print(('{} --> {} {} {} {}'.format(grid_params, train_accuracy[i],
                                           test_accuracy[i], train_f1[i],
                                           test_f1[i])))
...
class model_perf(object):
    ...
    def show(s, fontsize=None):
        ...
        for name in sorted(s.names):
            print((
                '{:5.2f} {:5.2f}   {:5.2f} {:5.2f}   {:.2e} {:.2e}   {:3.0f}   {}'
                .format(s.test_accuracy[name], s.train_accuracy[name],
                        s.test_f1[name], s.train_f1[name], s.test_loss[name],
                        s.train_loss[name], s.fit_time[name] * 1000, name)))
